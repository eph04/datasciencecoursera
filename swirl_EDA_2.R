...
| pollution. These Standards say that for fine particle pollution (PM2.5), the "annual
...
| and the longitude and latitude of the area. Now run the R command dim with pollution as
...
| these 5 numbers we see the Mean or average measure of particulate pollution across the
| 576 counties.
...
| at the pm25 column. We can also look at other information. To remind yourself what's
| there run the R command names with pollution as the argument.
...
| subset which, not surprisingly, "returns subsets of vectors, matrices or data frames
| which meet conditions". We'll use subset to pull off the data we want to plot. Call
| subset now with pollution as its first argument and a boolean expression testing region
| for equality with the string "east". Put the result in the variable east.
...
> hist(subset(x = pollution, region=="west")$pm25, col = "green")
...
| typing "pollution$" before the arguents to plot, so it saves us some typing and adds to
...
2: Explore basic questions and hypotheses (and perhaps rule them out)
