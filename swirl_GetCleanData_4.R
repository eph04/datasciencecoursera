...
| Wickham. According to the package authors, "lubridate has a consistent, memorable
| syntax, that makes working with dates fun instead of frustrating." If you've ever
| worked with dates in R, that statement probably has your attention.
...
| DESCRIPTION, a list of available functions, and a link to the official package
| vignette.
...
| There are three components to this date. In order, they are year, month, and day. We
| can extract any of these components using the year(), month(), or day() function,
...
> this_moment
[1] "2015-08-15 17:28:07 CEST"

| Keep up the great work!
  
  |==================                                                              |  23%

| Just like with dates, we can extract the year, month, day, or day of week. However, we
| can also use hour(), minute(), and second() to extract specific time information. Try
| any of these three new functions now to extract one piece of time information from
| this_moment
...
| today() and now() provide neatly formatted date-time information. When working with
| dates and times 'in the wild', this won't always (and perhaps rarely will) be the case.
...
| It looks almost the same, except for the addition of a time zone, which we'll discuss
| later in the lesson. Below the surface, there's another important change that takes
| place when lubridate parses a date. Type class(my_date) to see what that is.
...
| So ymd() took a character string as input and returned an object of class POSIXct. It's
| not necessary that you understand what POSIXct is, but just know that it is one way
...
| month, then the day. Hence, we used ymd() to parse them. What do you think the
...
| We can even throw something funky at it and lubridate will often know the right thing
| to do. Parse 25081985, which is supposed to represent the 25th day of August 1985. Note
| that we are actually parsing a numeric value here -- not a character string -- so leave
| off the quotes.
...
| something more ambiguous. Surround the number with quotes again, just to be consistent
| with the way most dates are represented (as character strings).
...
| You got a warning message because it was unclear what date you wanted. When in doubt,
| it's best to be more explicit. Repeat the same command, but add two dashes OR two
| forward slashes to "192012" so that it's clear we want January 2, 1920.
...
| In addition to dates, we can parse date-times. I've created a date-time object called
| dt1. Take a look at it now.
...
| lubridate is also capable of handling vectors of dates, which is particularly helpful
| when you need to parse an entire column of data. I've created a vector of dates called
| dt2. View its contents now.
...
| example, let's say the current time is 08:34:55 (hh:mm:ss). Update this_moment to the
| new time using the following command:
  | 
  | update(this_moment, hours = 8, minutes = 34, seconds = 55).

> update(this_moment, hours = 8, minutes = 34, seconds = 55)
[1] "2015-08-15 08:34:55 CEST"

| You got it right!
  
  |============================================                                    |  54%

| It's important to recognize that the previous command does not alter this_moment unless
| we reassign the result to this_moment. To see this, print the contents of this_moment.
...
| Unless you're a superhero, some time has passed since you first created this_moment.
| Use update() to make it match the current time, specifying at least hours and minutes.
| Assign the result to this_moment, so that this_moment will contain the new time.

> this_moment <- update(this_moment, hours = 17, minutes = 41, seconds = 55)
...
> this_moment
[1] "2015-08-15 17:41:55 CEST"

| You are doing so well!
  
  |================================================                                |  60%

| Now, pretend you are in New York City and you are planning to visit a friend in Hong
...
| in a variable called nyc. Check out ?now if you need help.
...
| The arrive variable contains the time that it will be in New York when you arrive in
| Hong Kong. What we really want to know is what time is will be in Hong Kong when you
| arrive, so that your friend knows when to meet you.
...
| Print the value of arrive to the console, so that you can tell your friend what time to
| pick you up from the airport.

> arrive
[1] "2015-08-18 21:24:53 HKT"
...
| the beginning of this lesson. This time, however, you should specify an extra argument,
| tz = "Singapore". Store the result in a variable called last_time.
...
| Pull up the documentation for new_interval(), which we'll use to explore how much time
| has passed between arrive and last_time.
...
| Create a new_interval() that spans from last_time to arrive. Store it in a new variable
| called how_long.
...
> as.period(how_long)
[1] "7y 2m 1d 21H 24M 53.9342041015625S"
Warning message:
...
| This concludes our introduction to working with dates and times in lubridate. I created
| a little timer that started running in the background when you began this lesson. Type
| stopwatch() to see how long you've been working!

> stopwatch()
[1] "1d 19H 48M 24.1778600215912S"
